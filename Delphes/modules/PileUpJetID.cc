...
void PileUpJetID::Process()
{
  ...
  while((candidate = static_cast<Candidate*>(fItJetInputArray->Next())))
  {
	...
	if (constituent->Charge == 0) {
	  ...
	  if (constituent->IsRecoPU > 0) {
	    sumptchpu += pt;
	  } else if (constituent->IsRecoPU == 0) {
	    sumptchpv += pt;
	  }
	  sumptch += pt;
	  nc++;
	...
	float tow_sumT = 0;
	float tow_sumW = 0;
	for (unsigned i = 0 ; i < constituent->ecal_E_t.size() ; i++) {
    ...
    candidate->SumPtCh = sumptch;
    candidate->SumPtChPV = sumptchpv;
    candidate->SumPtChPU = sumptchpu;
    if (sumptch > 0.) {
