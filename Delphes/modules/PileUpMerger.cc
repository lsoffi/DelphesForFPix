...
void PileUpMerger::Process()
{
  ...
  Double_t outputBSX = 0.0, outputBSY = 0.0;

  if (fBeamSpotInputArray->GetSize ())
    {
      Candidate *bs = (Candidate *) fBeamSpotInputArray->At (0);
      outputBSX = bs->Position.X ();
      outputBSY = bs->Position.Y ();
    }
  ...
  if (fPileUpDistribution == 0)
    poisson = gRandom->Poisson(fMeanPileUp);
  else if (fPileUpDistribution == 1)
    poisson = gRandom->Uniform(2.0 * fMeanPileUp);
  else
    poisson = fMeanPileUp;
  ...
  for(event = 0; event < poisson; ++event)
  {
    ...
    while(fReader->ReadParticle(pid, x, y, z, t, px, py, pz, e))
    {
      ...
      candidate->Position += TLorentzVector(outputBSX,outputBSY,0.,0.);
