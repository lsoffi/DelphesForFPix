...
void TrackPileUpSubtractor::Process()
{
  ...
  for(itInputMap = fInputMap.begin(); itInputMap != fInputMap.end(); ++itInputMap)
  {
    ...
    while((candidate = static_cast<Candidate*>(iterator->Next())))
    {
      ...
      if(fZVertexResolution < 0.0)
        isRecoPU = candidate->ClusterIndex >= 0 ? candidate->ClusterIndex != pv->ClusterIndex : -1;
      else
        isRecoPU = TMath::Abs(z-PVZ) > fZVertexResolution;;
      if(isRecoPU > 0) {
	candidate->IsRecoPU = 1;
      } else {
	candidate->IsRecoPU = isRecoPU;
	array->Add(candidate);
      }
    }
  }
